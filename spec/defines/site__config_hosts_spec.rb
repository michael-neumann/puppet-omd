...
describe 'omd::site::config_hosts' do
  ...
  it do
    is_expected.to contain_omd__site__config_hosts('default - a_default')
  end
  ...
  context 'with parameter cluster => true, cluster_tags => [tag1, tag2]' do
    ...
    it do
      is_expected.to_not contain_concat__fragment("default site's folder/hosts.mk COMMENT OUT clusters begin")
    end

    it do
      is_expected.to_not contain_concat__fragment("default site's folder/hosts.mk COMMENT OUT clusters end")
    end
  end
  ...
  context 'with parameter cluster => true, cluster_tags => breakme' do
    ...
    let(:params) do
      {
        :cluster => true,
        :cluster_tags => 'break me'
      }
    end

    it do
      is_expected.to raise_error(/is not an Array/)
    end
  end
